 ...
 */
@Controller
public class SysLoginController extends MvcHelper {
	
	Log log=LogFactory.getLog(SysLoginController.class);
	...
	public AjaxReturnMsg login(HttpServletRequest request,SysUser suer) {
		...
		try {
			subject.login(token);
			SysLoginInf inf=new SysLoginInf();
			...
			sysloginservice.saveLoginHashInfo(inf);
